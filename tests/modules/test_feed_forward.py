...
def test_forward_pass_shape(ff: FeedForwardLayer, settings: BertSettings):
    batch_size, seq_len = 2, 10
    input_tensor = torch.randn(batch_size, seq_len, settings.hidden_size)
    output_tensor = ff(input_tensor)
    assert output_tensor.shape == input_tensor.shape
    assert output_tensor.shape == (batch_size, seq_len, settings.hidden_size)


def test_residual_connection(ff: FeedForwardLayer, settings: BertSettings):
    ff.eval()
    ...
    input_tensor = torch.randn(1, 5, settings.hidden_size)
    output = ff(input_tensor)
    expected_norm = nn.functional.layer_norm(
        input_tensor, (settings.hidden_size,), eps=settings.layer_norm_eps
    )
    assert torch.allclose(output, expected_norm, atol=1e-6)


def test_dropout_is_applied(ff: FeedForwardLayer, settings: BertSettings):
    assert isinstance(ff.dropout, nn.Dropout)
    assert ff.dropout.p == settings.hidden_dropout_prob
